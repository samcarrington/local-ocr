...
| P2 DeepSeek mlx-vlm adapter | in-progress | `deepseek-ocr-vlm` passed an IDE end-to-end review flow but later reproduced mlx-vlm's continuous-batching cross-thread GPU-stream failure and model output without word boundaries. `scripts/mlx-vlm-server.py` transfers DeepSeek request tensors through host memory before the GPU-thread handoff and decodes DeepSeek output with the bundled fast-tokenizer metadata, while keeping NuExtract3 on the upstream path in the same process. A direct request retains word boundaries, but model output can still omit them for individual pages. The service now rejects that malformed DeepSeek output without overwriting the pending page; frontend page-28 validation remains outstanding. |
