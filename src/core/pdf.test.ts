...
const NO_TRANSFORM: [number, number, number, number, number, number] = [
  1, 0, 0, 1, 0, 0,
];
...
describe('computeImageRegions', () => {
  ...
  it('applies a y-flipping viewport transform', () => {
    ...
    expect(
      computeImageRegions(ops, [1, 0, 0, -1, 0, 1000], 1000, 1000),
    ).toEqual([[10, 930, 110, 980]]);
  });
  ...
  it('clamps regions to the device bounds', () => {
    ...
    expect(computeImageRegions(ops, NO_TRANSFORM, 500, 1000)).toEqual([
      [0, 20, 500, 70],
    ]);
  });
  ...
  it('expands repeated image xobjects from scale and positions', () => {
    const ops = makeOpList([
      ...
      [
        ...
        ['img_1', 100, 50, new Float32Array([10, 20, 200, 100])],
      ],
    ]);
    ...
    const ops = makeOpList([
      ...
      [
        ...
        [
          ...
          20,
          50,
          new Float32Array([10, 20, 200, 100]),
        ],
      ],
    ]);
  ...
  it.each([
    ...
    const ops = makeOpList([
      [OPS.paintImageXObjectRepeat, ['img_1', 100, 50, positions]],
    ]);
