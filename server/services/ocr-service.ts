...
export function createOcrService(config: AppConfig, overrides: Partial<OcrDependencies> = {}): OcrService {
  ...
  return {
    ...
    async rerun(id, number, body) {
      ...
      let result: { markdown: string; confidence?: number; figures?: DraftPage['figures']; layoutBlocks?: DraftPage['layoutBlocks'] }; let engine: DraftPage['engine'];
      if (job.kind === 'document') { try { result = await deps.convertDocumentToMarkdown(job.sourceFilePath); } catch (error) { throw conversionError(error); } engine = 'anydoc'; }
      else { const selected = engineName(object(body).engine); result = await rerunPage(deps.createAdapterRegistry(config).getAdapter(selected), page.imagePath!, selected); engine = selected; }
      page.markdown = result.markdown.trim(); page.confidence = result.confidence; page.figures = result.figures ?? page.figures; page.layoutBlocks = result.layoutBlocks; page.engine = engine; page.accepted = false; page.status = 'pending'; page.qualityWarnings = mergeWarnings(page, coverageWarning(page.nativeText, result.markdown, page.pageNumber)); job.updatedAt = new Date().toISOString(); await deps.saveJob(config, job); return { job, page };
    },
...
function engineName(value: unknown): EngineName { const engine = requiredString(value, 'engine'); if (!['tesseract', 'deepseek-ocr', 'glm-ocr', 'nuextract3-ocr'].includes(engine)) throw createApiError(400, `Unsupported engine: ${engine}`); return engine as EngineName; }
